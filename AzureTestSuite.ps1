...
Import-Module .\TestLibs\CILibs.psm1 -Force
...
Function RunTestsOnCycle ($cycleName , $xmlConfig, $Distro )
{
	...
	# If this is a CI environment, start JUnit report logger.
	$CIReportFolder = "$pwd/../CI"
	if(Test-Path $CIReportFolder)
	{
		CIStartLogReport("$CIReportFolder/report_$($testCycle.cycleName).xml")
	}
...
#		Set-Variable -Name testCaseRunObj -Value $testCaseRunObj -Scope Global
		...
		if ($currentTestData)
		{
			$testcase = CIStartLogTestCase $testsuite "$($test.Name)" "CloudTesting.$($testCycle.cycleName)"
...
#					AddTestCaseDetailsinDB -conn $conn -testCaseObj $testCaseRunObj -testSuiteRunId $testSuiteRunObj.testSuiteRunId
					...
					elseif($testResult -imatch "FAIL")
					{
						...
						CIFnishLogTestCase $testcase "FAIL" "$($test.Name) fail"
					}
					elseif($testResult -imatch "ABORTED")
					{
						...
						CIFnishLogTestCase $testcase "ERROR" "$($test.Name) abort"
					}
