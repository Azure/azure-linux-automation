	...
	foreach ($mode in $currentTestData.TestMode.Split(",")) 
	{
		...
		try
		{
			...
			{#.........................................................................Client command will decided according to TestMode....
				$cmd3="./start-client.py -c $hs1VIP -p $hs1vm1tcpport -t10 -P2" 
			}
			if(($mode -eq "URL") -or ($mode -eq "Hostname"))
			{
				$cmd3="./start-client.py -c $hs1ServiceUrl -p $hs1vm1tcpport -t10 -P2"
			}
			$client.cmd = $cmd3
...
#Verify Custom Probe Messages on both server
							...
							If (( IsCustomProbeMsgsPresent -logFile $server1Log -beg "Test Started" -end "TestComplete") -and (IsCustomProbeMsgsPresent -logFile $server2Log -beg "Test Started" -end "TestComplete")) {
								...
								If ((($diff/2)*100) -lt 20) {
									...
									$testResult = "FAIL"
									LogMsg "Connection Counts are not distributed correctly"
									LogMsg "Diff between server1 and server2 is $diff"
								}
							} else {
							...
							$testResult = "FAIL"
							LogMsg "Server is not Connected to Client"
						}
					} else {
						$testResult = "FAIL"
						LogMsg "Client is not Connected to Client"
					}	
				} else {
