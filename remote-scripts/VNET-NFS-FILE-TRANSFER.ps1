    ...
    foreach ($Value in $SubtestValues) 
    {
        ...
        #mkdir $LogDir\$Value\$mode -ErrorAction SilentlyContinue | out-null
            try
            {
                ...
                {
	                $testResult = DoNfsShareFileTranferTest -fromVM $fromVM -nfsServer $nfsServer -nfsServerDirctory '/nfsshare'
	            }

                if(($mode -eq "URL") -or ($mode -eq "Hostname"))
                {
	                $testResult = DoNfsShareFileTranferTest -fromVM $fromVM -nfsServer $nfsServer -nfsServerDirctory '/nfsshare' -HostnameMode
	            }
