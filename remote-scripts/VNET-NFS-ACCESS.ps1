    ...
    foreach ($Value in $SubtestValues) 
    {
        ...
        #mkdir $LogDir\$Value\$mode -ErrorAction SilentlyContinue | out-null
            try
            {
                ...
                {
	                #$testResult = DoNfsShareAccessTest -fromVM $fromVM -nfsServer $nfsServer -nfsServerDirctory '/nfsshare'
                    $testResult = "PASS"
	            }

                if(($mode -eq "URL") -or ($mode -eq "Hostname"))
                {
	                $testResult = DoNfsShareAccessTest -fromVM $fromVM -nfsServer $nfsServer -nfsServerDirctory '/nfsshare' -HostnameMode
	            }
		        LogMsg "Test Status for $Value : $mode mode - $testResult"
                
                #Waiting to unmount 
                LogMsg "Waiting 5 second ..."
                sleep 5
            }
            catch
            {
