    ...
    foreach ($Value in $SubtestValues) 
    {
        ...
        mkdir $LogDir\$Value\$mode -ErrorAction SilentlyContinue | out-null
            try
            {
                ...
                {
	                $testResult = DoMysqlOperationsTest -fromVM $fromVM -mysqlServer $mysqlServer -MysqlUsername $mysqlUsername -MysqlPassword $mysqlPassword
	            }

                if(($mode -eq "URL") -or ($mode -eq "Hostname"))
                {
	                $testResult = DoMysqlOperationsTest -fromVM $fromVM -mysqlServer $mysqlServer -MysqlUsername $mysqlUsername -MysqlPassword $mysqlPassword -HostnameMode
	            }
...
#$Cleanupcmd = "/home/$user/CleanupDnsServer.py"
#RunLinuxCmdOnRemoteVM -intermediateVM $intermediateVM -remoteVM $dnsServer -runAsSudo -remoteCommand $Cleanupcmd 
