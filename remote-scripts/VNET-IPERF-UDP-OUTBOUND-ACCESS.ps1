...
#region DEFINE LOCAL NET VMS
		...
		$externalServerUDPport = $xmlConfig.config.Azure.Deployment.Data.ExternalPingServer.UDPport
...
#region TEST EXECUTION  
		...
		foreach ($Value in $SubtestValues) 
		{
			...
			{ 
				...
				try
				{
					$udpServer = CreateIperfNode -nodeIp $externalServerIP -nodeSshPort $externalServerSSHport -user $externalServerUser -password $externalServerPass -nodeUdpPort $externalServerUDPport -nodeIperfCmd "./start-server.py -i1 -p $externalServerUDPport -u yes && mv Runtime.log start-server.py.log"
					switch ($Value)
					{
						"HS1VM1" {
