...
{  
    ...
    try
    {
        ...
        Function GenerateLoadToCpu ($IP, $sshPort, $user, $password)
        {
            LogMsg "Operation : Setup Kernbench : STARTED."
            $out = RunLinuxCmd -username $user -password $password -ip $IP -port $sshPort -command "chmod +x *.sh" 
            $out = RunLinuxCmd -username $user -password $password -ip $IP -port $sshPort -command "./SetupKernbench.sh" 
            LogMsg "Operation : Setup Kernbench : FINISHED."
            LogMsg "Operation : Start Kernbench : STARTED."
            $out = RunLinuxCmd -username $user -password $password -ip $IP -port $sshPort -command "./InvokeKernbench.sh" 
            LogMsg "Operation : Start Kernbench : FINISHED."
        }
        GenerateLoadToCpu -IP $hs1VIP -sshPort $hs1vm1sshport -user $user -password $password
